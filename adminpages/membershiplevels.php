	...
	if($action == "save_membershiplevel")
	{
		...
		if($saveid > 0)
		{
			$sqlQuery = $wpdb->prepare("
				...
				SET name = %s, description = %s, confirmation = %s, initial_payment = %f, billing_amount = %f, cycle_number = %d, cycle_period = %d, billing_limit = %f, trial_amount = %f, trial_limit = %d, expiration_number = %d, expiration_period = %d, allow_signups = %d
				WHERE id = %d LIMIT 1",
				$ml_name,
				$ml_description,
				$ml_confirmation,
